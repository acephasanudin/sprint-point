...
export function TableTasks() {
    ...
    const { data: tasks, isLoading, isError, isSuccess } = api.task.all.useQuery({ id, profileId, sprintId, teamId });
    ...
    const { mutate: findTask } = api.task.findTask.useMutation({
        ...
        onSuccess: (_, taskId) => {
            router.push(`/admin/points/${taskId}`);
        },
        onError: () => {
            setId(() => "");
            toast(
    ...
    const findListTask = (ids: Array<string>) => {
        setIsFilter(() => false);
        localStorage.setItem("taskIds", ids.toString());
        findTask(ids[0] || "");
    }
        ...
        <>
            <div className="navbar bg-base-100">
                ...
                <div className="navbar-center">
                    <input value={id} onPaste={(e) => { if (/\n/.test(e.clipboardData.getData('text'))) { findListTask(e.clipboardData.getData('text').split('\n')) } else { findTask(e.clipboardData.getData('text').replace('https://app.clickup.com/t/', '').replace('#', '')) } }} id="task-id" onChange={(e) => { if (!(/\s/.test(e.target.value)) && e.target.value.length > 5 || e.target.value.length === 0) setId(e.target.value.replace('https://app.clickup.com/t/', '').replace('#', '')) }} type="text" placeholder="Link / Clickup ID ..." className="input w-full" />
                </div>
                <Themes />
            </div>
